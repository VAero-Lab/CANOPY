...
def main():
    ...
    spec_fw = fw.TrunkSpec(
        chord_frac=0.5, span_cov=1.0, thick=0.004,
        stations=fw.make_mixed_stations(14),
        allow_crossing=True,
    )
    gen2 = fw.TreeGenerator(wing_fw)
    ...
    fig = viz_fw.view3d(br_fw, f'Full-wing mixed mode ({st2["n"]} webs)')
    fig.savefig(f'{OUT}/ex02_3d_fw.png', dpi=150, bbox_inches='tight')
    plt.close(fig)
